...
class New_Year(commands.Cog):
    ...
    async def check_time(self):
        ...
        for guild in self.bot.guild:
            ...
            db_guild = await self.db.fetch("SELECT * FROM config WHERE guild_id = $1", guild.id)
            reminder_message = await self.db.fetch("SELECT * FROM reminder_mesage WHERE guild_id = $1")
            reminder_message = await guild.get_channel(reminder_message["channel_id"]).fetch_message(reminder_message["message_id"])
            new_year_message = await self.db.fetch("SELECT * FROM new_year_message WHERE guild_id = $1")
            ...
            if estimated_left.total_seconds() in range(0,86400*2): # new year is here and it lasts for 2 days after that 
                ...
                try:
                    new_year_message.get_partial_message(new_year_message["message_id"]).delete()
                except:
                    pass
                await reminder_message.edit(
                    embed=(discord.Embed(
                        ...
                        color=discord.Color.green(),
                    )).set_footer(text=f"This message is controlled by timezone that's configured by the owner: {current_timezone.zone}")
                )
